...
internal sealed class EntityCache
{
    ...
    public IReadOnlyList<Guild> SnapshotGuilds() => _guilds.Values.OrderBy(g => g.Id, StringComparer.Ordinal).ToArray();
    ...
    public bool TryGetGuild(string guildId, out Guild guild) => _guilds.TryGetValue(guildId, out guild!);
}
